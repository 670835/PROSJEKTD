...
if not smarthouse.get_device_by_id("6b1c5f6b-37f6-4e3d-9145-1cfbe2f1fc28"):
    bulb = Actuator(
        id="6b1c5f6b-37f6-4e3d-9145-1cfbe2f1fc28",
        model_name="SimBulb 1.0",
        supplier="Simulate Inc.",
        device_type="LightBulb"
    )
    floor = smarthouse.register_floor(1)
    room = smarthouse.register_room(floor, room_size=10.0, room_name="Kjøkken")
    smarthouse.register_device(room, bulb)

    # 💾 Lagre til databasen så det huskes neste gang
    repo.insert_device(bulb)

# Registrer sensor manuelt hvis den ikke finnes
from smarthouse.domain import Sensor

if not smarthouse.get_device_by_id("4d8b1d62-7921-4917-9b70-bbd31f6e2e8e"):
    sensor = Sensor(
        id="4d8b1d62-7921-4917-9b70-bbd31f6e2e8e",
        model_name="SimTemp 1.0",
        supplier="Simulate Inc.",
        device_type="TemperatureSensor",
        unit="C"
    )

    # Opprett etasje og rom, og plasser sensoren
    floor = smarthouse.register_floor(1)
    room = smarthouse.register_room(floor, room_size=12.5, room_name="Stue")
    smarthouse.register_device(room, sensor)
